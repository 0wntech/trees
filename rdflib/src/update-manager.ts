...
export default class UpdateManager {
  ...
  editable(
    ...
    if ((uri as string).slice(0, 8) === "file:///") {
      if (
        kb.holds(
          ...
          this.store.rdfFactory.namedNode(
            "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"
          ),
          this.store.rdfFactory.namedNode(
            "http://www.w3.org/2007/ont/link#MachineEditableDocument"
          )
        )
      ) {
        return "LOCALFILE";
    ...
    var requests = kb.each(
      undefined,
      this.ns.link("requestedURI"),
      // @ts-ignore
      docpart(uri)
    );
    var method: string;
  ...
  cacheIfps() {
    ...
    a = this.store.each(
      undefined,
      this.ns.rdf("type"),
      this.ns.owl("FunctionalProperty")
    );
    for (let i = 0; i < a.length; i++) {
  ...
  mentioned(x) {
    return (
      this.store.statementsMatching(x, null, null, null).length !== 0 || // Don't pin fresh bnodes
  ...
  fire(
    ...
    return Promise.resolve()
      .then(() => {
        if (!uri) {
          throw new Error(
            "No URI given for remote editing operation: " + query
          );
        }
      ...
      .then((response) => {
        if (!response.ok) {
          let message =
            "UpdateManager: update failed for <" +
            uri +
            "> status=" +
            response.status +
            ", " +
  ...
  updateMany(
    ...
    const updates = uniqueDocs.map((doc) =>
      thisUpdater.update(
        deletions.filter((st) => st.why.equals(doc)),
  ...
  update(
    ...
    if (!callback) {
      ...
      return new Promise(function (resolve, reject) {
        // Promise version
        thisUpdater.update(deletions, insertions, function (
          uri,
          ok,
          errorBody
        ) {
    ...
    try {
      ...
      var ds = !deletions
        ? []
        : isStore(deletions)
        ? deletions.statements
        : deletions instanceof Array
        ? deletions
        : [deletions];
      var is = !insertions
        ? []
      ...
      if (!doc) {
        let message =
          "Error patching: statement does not specify which document to patch:" +
          ds[0] +
          ", " +
          is[0];
        // console.log(message)
      ...
      if (protocol === undefined) {
        ...
        if (secondTry) {
          throw new Error(
            "Update: Loaded " +
              doc +
              "but stil can't figure out what editing protcol it supports."
          );
        }
        // console.log(`Update: have not loaded ${doc} before: loading now...`);
        (this.store.fetcher.load(doc) as Promise<Response>).then(
          (response) => {
            this.update(deletions, insertions, callback, { secondTry: true });
          },
          (err) => {
            if (err.response.status === 404) {
              ...
              throw new Error(
                `Update: Can't get updatability status ${doc} before patching: ${err}`
              );
            }
          }
        );
        ...
        control.pendingUpstream = control.pendingUpstream
        ...
        this.fire(doc.value, query, (uri, success, body, response) => {
          ...
          console.log(
            "    UpdateManager: Return " +
              (success ? "success " : "FAILURE ") +
              (response as Response).status +
              " elapsed " +
              (response as any).elapsedTimeMs +
              "ms"
          );
          if (success) {
            try {
        ...
        if ((protocol as string).indexOf("LOCALFILE") >= 0) {
          ...
          throw new Error(
            "Unhandled edit method: '" + protocol + "' for " + doc
          );
        }
      }
    } catch (e) {
      callback(
        undefined,
        false,
        "Exception in update: " + e + "\n" + Util.stackString(e)
      );
    }
  }

  updateDav(doc: Quad_Subject, ds, is, callbackFunction): null | Promise<void> {
    let kb = this.store;
    ...
    var response = kb.any(
      request as NamedNode,
      this.ns.link("response")
    ) as Quad_Subject;
    if (!response) {
      return null; // throw "No record HTTP GET response for document: "+doc
    }
    var contentType = (kb.the(response, this.ns.httph("content-type")) as Term)
      .value;
    ...
    return kb.fetcher
      .webOperation("PUT", targetURI, options)
      .then((response) => {
        if (!response.ok) {
  ...
  updateLocalFile(doc: NamedNode, ds, is, callbackFunction): void {
    ...
    if (!contentType) {
      throw new Error(
        "File extension ." + ext + " not supported for data write"
      );
    }
    ...
    if (!file.exists()) {
      throw new Error(
        "Rewriting file <" + doc.value + "> but it does not exist!"
      );
    }
    // {
  ...
  reload(
    ...
    (kb as any).fetcher.nowOrWhenFetched(doc.value, options, function (
      ok: boolean,
      body: Body,
      response: Response
    ) {
      if (!ok) {
        ...
        callbackFunction(
          false,
          "Non-HTTP error reloading data: " + body,
          response
        );
      } else {
